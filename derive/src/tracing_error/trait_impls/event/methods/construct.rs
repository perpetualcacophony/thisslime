...
pub fn quote(input: &syn::DeriveInput) -> syn::Result<syn::ImplItemFn> {
    match input.data {
        syn::Data::Struct(ref data) => {
            let attr = core::tracing::attrs::Event::from_attributes(&input.attrs)?;

            let tracing_fields = data
                .fields
                .iter()
                .map(core::tracing::model::Field::try_from)
                .collect::<Result<_, syn::Error>>();

            if let Ok(tracing_fields) = tracing_fields {
                let event = core::tracing::model::Event::new(attr.level, tracing_fields, true);
